 ...
 * commit info: Jun 8, 2021 #e7c6c1c3009f0e9c
 *
 */
...
namespace std {
    ...
    template <typename _T> using __iter_reference_t = decltype(*std::declval<_T&>());

    template <typename _T> using iter_value_t = typename std::iterator_traits<std::remove_cvref_t<_T>>::value_type;
    ...
    namespace ranges {
        ...
        template <typename _Range> using iterator_t = decltype(begin(std::declval<_Range>()));

        template <typename _Range> using sentinel_t = decltype(end(std::declval<_Range>()));

        template <typename _Range> using range_reference_t = __iter_reference_t<iterator_t<_Range>>;

        template <typename _Range> using range_value_t = iter_value_t<iterator_t<_Range>>;
...
namespace std {
    ...
    namespace ranges {
        ...
        template <typename _Rng, typename _Allocator = use_allocator_arg> struct elements_of {
        ...
        template <typename _Rng> elements_of(_Rng&&) -> elements_of<_Rng>;

        template <typename _Rng, typename Allocator> elements_of(_Rng&&, Allocator&&) -> elements_of<_Rng, Allocator>;
    ...
    template <typename _Ref> struct __generator_promise_base {
        ...
        template <typename _Gen> struct __yield_sequence_awaiter {
            _Gen __gen_;
    ...
    struct __generator_promise<generator<_Ref, _Value, _Alloc>, _ByteAllocator, _ExplicitAllocator> final
        ...
        {
            static_assert(!_ExplicitAllocator, "This coroutine has an explicit allocator specified with "
    ...
    template <typename _Ref, typename _Value, typename _Alloc> class generator
    {
        using __byte_allocator = __byte_allocator_t<_Alloc>;
      ...
      public: // to get around access restrictions for __yield_sequence_awaitable
        std::coroutine_handle<> __get_coro() noexcept
        {
            return __coro_;
        }
        promise_type* __get_promise() noexcept
        {
    ...
    template <typename _Ref, typename _Value> class generator<_Ref, _Value, use_allocator_arg>
    {
      ...
      public: // to get around access restrictions for __yield_sequence_awaitable
        ...
        __promise_base* __get_promise() noexcept
        {
            return __promise_;
        }
